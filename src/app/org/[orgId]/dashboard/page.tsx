...
import { Target, Users, ClipboardList, TrendingUp, AlertTriangle, CheckCircle2, User as UserIcon, Bell, Zap } from "lucide-react";
...
export default async function OrgDashboardPage(props: { params: Promise<{ orgId: string }> }) {
    ...
    const topPerformers = (members || [])
        .map((m: any) => {
            ...
            return { name: m.profiles?.full_name || 'Unknown', score, done, total };
        })
    ...
    return (
        <div className="p-6 md:p-8 max-w-screen-xl mx-auto animate-in fade-in duration-500">
            <header className="mb-8">
                ...
                <p className="text-slate-500 text-sm">Overview of team productivity and goals</p>
            </header>

            {/* Friction Board — only show if blocked tasks exist */}
            {isAdmin && (blockedTasks?.length ?? 0) > 0 && (
                        ...
                        <Alert key={task.id} variant="destructive" className="bg-red-50 border-red-200 shadow-sm">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 w-full">
                                ...
                                <div className="flex gap-2">
                                    <AIPivotDialog taskId={task.id} orgId={orgId} taskTitle={task.title} />
                                    <form action={async () => {
                                        'use server';
                        ...
                        <div className="flex items-end justify-between gap-2 h-40 px-2">
                            {barHeights.map((h, i) => (
                                <div key={i} className="flex flex-col items-center gap-1.5 flex-1">
                                    <div
                                        className="w-full rounded-t-lg bg-[#22c55e] hover:bg-[#16a34a] transition-all duration-300 cursor-pointer"
                                        style={{ height: `${h}%` }}
                                    />
                    ...
                    <CardContent>
                        <div className="h-40 flex items-end">
                            <svg viewBox="0 0 300 100" className="w-full h-full" preserveAspectRatio="none">
                                ...
                                <path
                                    d={`M ${linePoints.map((p, i) => `${(i / (linePoints.length - 1)) * 300},${100 - p}`).join(' L ')}`}
                                    fill="none" stroke="#22c55e" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
                                />
                                <path
                                    d={`M 0,${100 - linePoints[0]} ${linePoints.map((p, i) => `L ${(i / (linePoints.length - 1)) * 300},${100 - p}`).join(' ')} L 300,100 L 0,100 Z`}
                                    fill="url(#lineGrad)"
                                />
                                {linePoints.map((p, i) => (
            ...
            {/* Bottom Row: Alerts + Top Performers */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Recent Alerts & Nudges */}
                <Card className="border-none shadow-sm">
                        ...
                        {(blockedTasks?.length ?? 0) > 0 ? (
                            blockedTasks!.slice(0, 3).map((task, i) => (
                                <div key={task.id} className="flex items-start gap-3 group animate-in fade-in duration-300" style={{ animationDelay: `${i * 100}ms` }}>
                                    ...
                                    <div>
                                        <p className="text-sm font-semibold text-slate-800">{(task as any).assigned_to_profile?.full_name} is blocked on "{task.title}"</p>
                            ...
                            <>
                                {[
                                    ...
                                    { icon: Zap, color: "text-blue-500 bg-blue-50", msg: "Product Launch tasks all completed on time", time: "1 day ago" },
                        ...
                        {topPerformers.length > 0 ? topPerformers.map((p, i) => {
                            ...
                            return (
                                <div key={i} className="flex items-center gap-3 group animate-in fade-in duration-300" style={{ animationDelay: `${i * 100}ms` }}>
                                    <div className={`${colors[i]} h-9 w-9 rounded-xl flex items-center justify-center text-white font-bold text-sm shrink-0`}>
                                        {initials}
                                    </div>
